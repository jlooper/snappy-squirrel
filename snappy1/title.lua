...
-- forward declaration
local background,snappyLeft,title,parents
tweenTime = 500
currPage = 1
...
local function playClick()
  sound = audio.play( click )
	...
	title.x, title.y = display.contentWidth/2, 40

	snappyLeft = display.newImageRect( sceneGroup, "images/snappy.png", 235,280 )
	snappyLeft.x, snappyLeft.y = display.contentWidth/2, display.contentHeight/2+30
	...
	local function goToPlay(event)
		if event.phase == "ended" then
			...
			composer.gotoScene( "play", "slideLeft", 800 )		
			return true
		end
	end
	...
	local function read( event )
	    ...
	    playClick()
	    audio.play(audioFile, { channel=2})
	...
	btnRead.x = display.contentWidth/2 - 150
